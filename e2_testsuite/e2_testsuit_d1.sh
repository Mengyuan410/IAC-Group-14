...
mkdir -p bin
set +e 
iverilog -Wall -g2012 -o rt1/m1 \ rt1/m1.v  #this is the fie to compile


rt1/m1 bin/m1.stdout
RESULT=$?
set -e

if [[ "${RESULT}" == 0 ]] ; then
   echo " rt1/m1.v PASS" #we want to check for a 0 exit code. If we get 0 as exit code it is fine. 
else
